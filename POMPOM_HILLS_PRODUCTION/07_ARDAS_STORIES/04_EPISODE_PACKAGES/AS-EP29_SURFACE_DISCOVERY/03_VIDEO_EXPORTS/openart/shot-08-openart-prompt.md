# Shot 08 — OpenArt Prompt — shot-08-rhyme-again

## Visual Prompt

Arda's Home garden transitioning toward Friendship Meadow, warm morning sunlight, garden path
leading to a wide inviting meadow visible in the distance, green grass, rounded tree. Arda
(energetic boy, brown curly hair, orange t-shirt, blue shorts) stands up, holds the small
round acorn — warm brown, smooth, 3 cm diameter — proudly in both hands, and repeats the
rhyme with a bigger smile. He turns toward the meadow path in the background and begins
walking. Warm morning dappled sunlight, soft shadows, rounded safe shapes, pastel colours, no
other characters, {style} {camera} {lighting}

Use @image1 as continuity from Shot 07.

The garden background is locked from the first frame; keep tree, grass, path, daisies, fence,
house, meadow visible in distance stable.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No text.

## Camera Direction

32mm wide at child eye level, framing Arda standing and reciting the rhyme. Camera follows
gently as he walks toward the meadow. Warm, steady framing. No fast zooms, no dramatic
angles.

## Sound

Soft outdoor garden ambience: gentle birds chirping. Gentle footsteps on grass. Arda's voice
is proud and rhythmic: "Smooth? Fast! Bumpy? Slow! Different surface, different roll!" Soft
breeze. Natural ambience only. No music, no background music, no melody, no song, no
soundtrack, no chimes, no whooshes.

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, photorealistic, horror, scary,
dark lighting, fast movement, sudden camera motion, static talking pose, characters frozen,
background music, music, melody, song, soundtrack, sharp edges, angular shapes, realistic
skin texture, realistic pores, high contrast, neon colors, HDR look, glossy plastic,
harsh shadows, blown highlights, extra characters, aggressive play, modern technology,
screens, cinematic LUT, blue tint, green tint, orange shift

## Voice Lock

Each character must use their registered locked voice from VOICE_TRACKER.md.
Arda uses Arda's locked voice. Luca uses Luca's locked voice. Noah uses Noah's locked voice.
Do not let OpenArt choose or generate voices.
Do not change voice between shots.
Same pitch, same timbre, same age, same speed, same warmth.

## OpenArt Settings

- Duration: 15 seconds
- Mode: Text-to-video
- Prompt enhancer: Off
- Cinematic/auto camera: Off
- Locked camera only

## Shot Approval Checklist

- [ ] First frame matches scene description
- [ ] Character identity preserved (Arda)
- [ ] No camera movement (locked camera)
- [ ] No dialogue as on-screen text
- [ ] No music, no background music
- [ ] Acorn held proudly
- [ ] Friendship Meadow visible in background
- [ ] Soft pastel preschool look maintained
- [ ] No contrast increase, no saturation drift
- [ ] Warm natural lighting preserved
- [ ] First 0.5s is stable and edit-safe
- [ ] Final 0.5s is calm and transition-safe
