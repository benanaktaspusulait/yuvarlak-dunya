# Learning Room — OpenArt World Builder Prompt
...
If World Builder's 3D output still drifts from our identity (wrong furniture shape, looks like a modern classroom, sharp corners), that is an inherent limitation of the extrapolation — not something a better prompt can fully fix. Treat any World Builder output as a rough 3D blockout, and run it through the Production QA checklist before treating any capture from it as canon.
...
- does not understand internal asset IDs like `LR-TABLE-01` or `LR-BOARD-01`
...
```
A warm wooden cottage interior with a big round activity table at the centre, surrounded by small round preschool chairs. A large round chalkboard on the back wall has simple friendly chalk drawings — a sun, a mountain, a butterfly. Arched windows let warm morning sunlight pour in. Low wooden shelves hold books facing outward, small baskets, and indoor plants. A cosy reading corner has a soft round rug, cushions, a small bookshelf, a lantern, and stuffed animals. A creative corner has paper, crayons, wooden blocks, and clay. Warm cream walls, warm wooden floor, round ceiling beams. Children's storybook style, soft matte handcrafted look, cozy and safe. No modern furniture, no metal, no plastic, no technology, no text, no sharp corners.
```
...
- Image 1 (front): wide interior from the doorway, showing the full room layout
- Image 2 (back): view from the chalkboard looking toward the entrance
- Image 3 (left): close-up of the reading corner with rug and books
- Image 4 (right, optional): close-up of the creative corner with art supplies

All four images must share the same warm lighting, cream colour palette, and wooden texture so OpenArt reads them as one consistent space.
