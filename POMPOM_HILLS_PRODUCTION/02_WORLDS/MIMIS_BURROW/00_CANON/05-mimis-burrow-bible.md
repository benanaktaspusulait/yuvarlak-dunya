# Environment Bible — Mimi's Burrow

> **Version 3.1** — Visual Richness & World Charm pass added.
> Mimi'nin yuvası, yeraltında güvenli bir sığınaktır. Çocuklar için "kendilerine ait bir yer" hissi.

---

```
Environment ID: ENV-005
File: POMPOM_HILLS_PRODUCTION/02_WORLDS/MIMIS_BURROW/
Version: 3.1
Location: Flower Hill — Home Cluster Zone
Type: Living World (Exterior + Interior)
Status: Design
Bible: ✅
Hero View: ❌
World Spec: ❌
```

---

## Overview

Mimi'nin yuvası, çimenliğin altında yer alan yuvarlak bir yeraltı girişidir. Üstünde çiçekler, içinde sıcak mavi ışık vardır. Güven ve rahatlık hissi verir — çocuklar için "kendilerine ait bir sığınak."

---

## Purpose

Mimi'nin yuvası güven ve rahatlık hissi verir. Küçük, sıcak, davetkar. Burada Mimi dinlenir, güvende hisseder. Çocuklar için "yuva" kavramının somut hali — yer altında, gizli, ama korkutucu olmayan bir dünya.

---

## Why This World Exists ⭐

Mimi's Burrow, yeraltında güvenli bir sığınak olarak tasarlanmıştır — children see a cozy, hidden retreat that feels like a secret world beneath the grass. The underground layout with warm blue tones and soft textures reinforces Mimi's gentle, introverted personality. This space provides contrast to Kiko's above-ground home, enriching the world's spatial diversity.

---

## Emotional Purpose

### Bu mekan duygusal olarak neden var?

"Gizli bir dünya" hissi. Yeraltında olmak korkutucu değil — sadece onlara ait bir sığınak.

### Çocuklar buraya geldiklerinde nasıl hisseder?

Güvenli, rahat, meraklı. "Burada herkes beni sever."

### Bu mekanda doğal olarak hangi hikayeler geçer?

- Mimi sabah uyanır, kulakları görünür
- Kiko ziyarete gelir, birlikte otururlar
- Yağmur yağar, içeride sessizce otururlar
- Mimi havuç yer
- Kiko ile kitap okurlar
- Saklanma oyunu oynarlar

### Bu mekan hangi duygusal deneyimleri destekler?

Güven · Rahatlık · Sakinlik · Dostluk · Gizli keşif

---

## Play Philosophy

### Oyun stili

| Özellik | Açıklama |
|---------|----------|
| Open-ended | Belirli aktivite yok, kendi oyunlarını yaratır |
| Hayal gücü | Kitap okuma, hayal kurma, sessiz oyun |
| İşbirliği | Birlikte oturma, birlikte dinlenme |
| Gözlem | Pencereden dışarıyı izleme |
| Yumuşak hareket | Yavaş hareket, yumuşak dokunuş |

### Kural tabanlı oyun yok

- Yarışma yok
- Skor yok
- Rekabet yok

### Doğal etkileşimler

- Kitap okuma
- Havuç yeme
- Pencereden dışarıyı izleme
- Uyuma
- Sessizce oturma
- Birlikte şarkı söyleme

---

## World Position

```
Kiko's Home (Flower Hill — Home Cluster Zone)
        ↓ 1 dakika
Mimi's Burrow (Flower Hill — Home Cluster Zone) ← BURASI
        ↓ 1 dakika
Rosie's Rose Garden
        ↓ 2 dakika
Butterfly Meadow
```

**Komşuları:**
- Kuzey: Kiko's Home (1 dk, aynı Home Cluster Zone)
- Güney: Rosie's Rose Garden (1 dk)
- Doğu: Flower Hill — Nature Field Zone (çiçek tarlası)
- Batı: Central Square (3 dk)

---

## Visual Identity

### Genel

- Yeraltında yuvarlak giriş
- Çimenliğin altında
- Üstünde çiçekler
- İçeride sıcak mavi ışık
- Yumuşak, peluş dokular
- Her şey yuvarlak ve küçük

### Giriş

- Biçim: Yuvarlak, mavi çerçeve (#90CAF9)
- Yükseklik: 0.80 m (Mimi için ideal)
- İçerisi: Sıcak ışık var
- Kapı: Yok (açık giriş)

### Üst Kısım

- Çimenlik: Yumuşak yeşil (#C8E6C9)
- Çiçekler: 3-4 küçük yuvarlak çiçek
- Pompom bush: 1 adet, küçük

### Yol

- Yuvarlak taşlar (#E0E0E0)
- Kapıya kadar gider
- 3 taş

### İç Mekan

- Biçim: Yuvarlak, yeraltı odası
- Duvarlar: Yumuşak mavi (#90CAF9)
- Tavan: Yuvarlak, beyaz
- Zemin: Yumuşak, yeşil halı

---

## Spatial Layout

### Dış

```
    Çiçekler
        ↓
    Çimenlik
        ↓
    Mavi Giriş → Yol → Patika
```

### İç

```
    Pencere (duvar)
        ↓
    Yastık → Mini raf
        ↓
    Havuç kutusu
        ↓
    Giriş (çıkış)
```

Her alan bir sonrakine yumuşak geçiş sağlar. Sert duvarlar veya ani kesimler yoktur.

---

## World Zones

### Giriş Dış Alanı

- **Purpose:** Yuvanın ilk karşılaştığı alan
- **Emotional Purpose:** Merak ve davet — "İçeri girmek ister misin?"
- **Typical Activities:** Çiçek gözlemleme, taş counting, arkadaş bekleme
- **Lighting:** Dış mekan aydınlatması, sıcak
- **Mood:** Davetkar, renkli, canlı
- **Important Props:** Çiçekler, taş yol, çimenlik
- **Camera Usage:** Wide establishing, entrance approach

### Giriş

- **Purpose:** Dış ile iç arasındaki köprü
- **Emotional Purpose:** Geçiş anı — "Artık yerin altındasın"
- **Typical Activities:** Girişten bakma, içeri girme
- **Lighting:** Yarı gölge, sıcak
- **Mood:** Sıcak, tanıdık
- **Important Props:** Yuvarlak mavi çerçeve
- **Camera Usage:** Door close-up, entrance shot

### İç Mekan

- **Purpose:** Yuvanın kalbi — tüm aktiviteler burada
- **Emotional Purpose:** Güven, rahatlık, huzur
- **Typical Activities:** Uyuma, kitap okuma, dinlenme, hayal kurma
- **Lighting:** İç mekan sıcak aydınlatması
- **Mood:** Sıcak, güvenli, sessiz
- **Important Props:** Yastık, pencere, mini raf, havuç kutusu
- **Camera Usage:** Wide interior, medium, close-up

---

## Props

| Prop | Asset ID | Açıklama | Reusable |
|------|----------|----------|:--------:|
| Giriş çerçevesi | MB-DOOR-01 | Yuvarlak mavi çerçeve (#90CAF9) | ✅ |
| Yastık | MB-PILLOW-01 | Yuvarlak mavi peluş yastık | ✅ |
| Pencere | MB-WINDOW-01 | Küçük yuvarlak pencere | ✅ |
| Mini raf | MB-SHELF-01 | Yuvarlak beyaz raf, kitaplar | ✅ |
| Havuç kutusu | MB-CARROT-BOX-01 | Yuvarlak turuncu kutu (#FF7043) | ✅ |
| Havuçlar | MB-CARROTS-01 | 2-3 turuncu havuç | ✅ |
| Çiçekler | MB-FLOWERS-01 | Üstteki küçük çiçekler | ✅ |
| Taşlar | MB-STONES-01 | Yuvarlak gri taşlar (#E0E0E0) | ✅ |
| Çimenlik | MB-GRASS-01 | Yumuşak yeşil çimen (#C8E6C9) | ✅ |
| Pompom bush | MB-BUSH-01 | Küçük yeşil çalı | ✅ |

---

## Camera Rules

### Genel

| Açı | Kullanım | Not |
|-----|----------|-----|
| Hero exterior | Giriş tanıtımı | Yuvarlak mavi çerçeve merkezde |
| Entrance close-up | Yakın giriş | İçeriden sıcak ışık görünür |
| Wide interior | Tüm iç mekan | Tüm eşyalar görünür |
| Medium | Yastık/havuç | Sıcak detay |
| Door POV | İçeriden dışarı | Dış görünüm |
| Close-up | Prop detayı | Kitap, havuç, yastık |

### Kamera Yüksekliği

- Dış: 0.80 m (Mimi göz hizası)
- İç: 0.60 m (düşük, yeraltı)
- Close-up: 0.40 m (obje detayı)

---

## Canonical Prompt Reference Pack

### Reference 01 — Hero View

**Purpose:** Master environment reference.

**Camera position:** Dışarıdan girişe bakan geniş açı. Çimenlik üstte, mavi giriş ortada.

**Framing:** 100% çevre. Karakter yok. Hayvan yok. Saf çevre referansı.

**Composition:**
- Üst: Çimenlik ve çiçekler
- Orta: Yuvarlak mavi giriş (#90CAF9)
- Alt: Taş yol (#E0E0E0)
- Arka: Sıcak ışık içeri sızıyor

**Required props:**
- MB-DOOR-01, MB-FLOWERS-01, MB-STONES-01, MB-GRASS-01

**Lighting:** Sıcak dış mekan ışığı, girişten içeri sıcak sızıntı. Yumuşak gölge.

**Mood:** Davetkar, sıcak, gizli, merak uyandırıcı.

**What must always be visible:**
- Yuvarlak mavi giriş
- Üstte çiçekler
- Taş yol
- Sıcak iç ışık

**What should never appear:**
- Karakter
- Karanlık
- Soğuk renkler
- Keskin köşe
- Büyük eşyalar
- Korkutucu öğe

### Reference 02 — Entrance View

**Purpose:** Girişin yakın çekimi.

**Camera position:** Girişin hemen önünde, içeriye bakan açı.

**Used for:**
- İçeri girme anları
- Mimi'nin kulaklarının görünmesi
- Sıcak karşılanma

### Reference 03 — Interior View

**Purpose:** İç mekanın genel görünümü.

**Camera position:** İçeriden, geniş açı.

**Used for:**
- İç mekan sahneleri
- Uyuma anları
- Kitap okuma

### Reference 04 — Detail View

**Purpose:** Prop detayları.

**Camera position:** Close-up çekimler.

**Composition:**
- Yastık detayı
- Havuç kutusu
- Mini raf ve kitaplar
- Pencere detayı

**Purpose for OpenArt:** Prop detayı ve malzeme referansı için.

---

## Prompt Generation Rules

| Kural | Açıklama |
|-------|----------|
| Hero View primary | Her shot'ta en fazla 4 referans kullanılır |
| Hero View zorunlu | Her üretimde Hero View yüklenir |
| Tutarlı ışık | Sıcak, yeraltı ışığı |
| Tutarlı renk | Mavi + yeşil + turuncu palet |
| Tutarlı malzeme | Yumuşak peluş, doğal taş |
| Tutarlı ölçek | Küçük, çocuk boyutu |
| Karakter yok referanslarda | Referanslar yalnızca ortam |

### Shot Oluşturma Sırası

```
1. Hero View yükle (zorunlu)
2. Entrance View ekle (gerekirse)
3. Interior View ekle (gerekirse)
4. Karakter referansı yükle
5. Shot oluştur
6. Reddetme kurallarını uygula
7. Kabul et veya yeniden üret
```

### Reddetme Kuralları

- Karanlık varsa reddet
- Soğuk renk varsa reddet
- Keskin köşe varsa reddet
- Büyük eşya varsa reddet
- Korkutucu öğe varsa reddet
- Giriş mavi çerçeve değilse reddet

---

## Soundscape

| Ses | Kaynak | Ne zaman |
|-----|--------|----------|
| Kulak flop | Mimi | Giriş/çıkış |
| Hop sesi | Mimi | Hareket |
| Sıcak ambient | İçeride | Her zaman |
| Kuş cıvıltısı | Dışarıdan | Sabah |
| Sessizlik | İçeride | Uyku zamanı |

---

## Animation Rules

### Giriş

| Aksiyon | Frame | Hız |
|---------|-------|-----|
| Kulak görünmesi | 10 frame | Yavaş |
| Mimi'nin çıkması | 12 frame | Hop |

### İçeride

| Aksiyon | Frame | Hız |
|---------|-------|-----|
| Yastık sallanması | 20 frame | Çok yavaş |

---

## Forbidden

```
✗ Karanlık
✗ Soğuk renkler
✗ Sivri köşe
✗ Büyük eşyalar
✗ Gürültü
✗ Korkutucu öğe
✗ Karakter (hero view'da)
✗ Metin veya logo
✗ Metal veya plastik
✗ Hızlı hareket
```

---

## Story Opportunities

Her fırsat doğal, sakin ve preschool'a uygundur:

1. Mimi sabah uyanır, kulakları görünür
2. Kiko ziyarete gelir, birlikte otururlar
3. Yağmur yağar, içeride sessizce otururlar
4. Mimi havuç yer
5. Kiko ile kitap okurlar
6. Gece yıldızlara bakarlar
7. Mimi uyur, Kiko izler
8. Saklanma oyunu oynarlar
9. Mimi yeni bir şey keşfeder
10. Birlikte şarkı söylerler
11. Mimi kulaklarını sallar
12. Kiko yastığa dokunur
13. Havuç kutusunu düzenlerler
14. Pencereden kuşları izlerler
15. Sessizce birlikte otururlar
16. Mimi Kiko'yu karşılar
17. Birlikte uyurlar
18. Sabah uyanırlar
19. Dışarı çıkarlar
20. Yuvaya geri dönerler

---

## Emotional Tone

- Güvenli
- Sıcak
- Sakin
- Gizli
- Rahat
- Dostça
- Huzurlu
- Sevimli
- Küçük
- Korumalı

---

## Production Notes

### Best Characters

- Mimi (ev sahibi)
- Kiko (ziyaretçi)
- Opa (nadiren, hikaye anlatımı)

### Avoid

- Karanlık sahneler
- Soğuk renkler
- Büyük eşyalar
- Gürültü
- Korkutucu öğeler

### Typical Duration

30–60s per scene

### Best Emotional Themes

- Güven ve rahatlık
- Sakinlik ve huzur
- Dostluk ve birlikte vakit geçirme
- Gizli keşif

### Typical Opening Shots

1. Çimenlikten aşağıya bakış, mavi giriş görünür
2. Girişten içeriye bakış, sıcak ışık
3. İç mekanın genel görünümü

### Typical Closing Shots

1. Mimi uyur, pencereden ay görünür
2. Girişten dışarıya bakış, gün batımı
3. İçeride sessizlik, sıcak ışık

### Camera Recommendations

| Durum | Lens | Yükseklik | Not |
|-------|------|-----------|-----|
| Dış genel | 28mm | 0.80m | Giriş görünür |
| Giriş | 35mm | 0.60m | Yakın çekim |
| İç mekan | 28mm | 0.60m | Tüm eşyalar |
| Close-up | 50mm | 0.40m | Prop detayı |

### Continuity Notes

- Giriş her zaman yuvarlak ve mavi
- İçeride her zaman sıcak ışık
- Yastık her zaman yerinde
- Havuç kutusu her zaman dolu
- Pencere her zaman açık

---

## Consistency Checklist

```
✓ Giriş yuvarlak ve mavi mi?
✓ İçeride yastık var mı?
✓ Havuç kutusu dolu mu?
✓ Pencere yuvarlak mı?
✓ Sıcak ışık var mı?
✓ Sivri köşe yok mu?
✓ Mimi'nin kulakları sığıyor mu?
✓ Çiçekler üstte mi?
✓ Taş yol var mı?
✓ Büyük eşya yok mu?
✓ Korkutucu öğe yok mu?
✓ Karanlık yok mu?
```

---

## Visual Richness & World Charm

> See `POMPOM_HILLS_PRODUCTION/00_GLOBAL_RULES/WORLD_STANDARDS/GLOBAL_ENVIRONMENT_STANDARD.md` § Visual Richness Principle.
> Mimi's Burrow must NOT be reduced to "a hole in the ground."

Mimi's Burrow is the cozy hidden hideaway of Pompom Hills. It should feel
like: **"I want to peek inside that little blue doorway."**

### Iconic Landmark

MB-DOOR-01 — the round blue entrance frame set beneath the grass, with
flowers on top and warm light spilling out. This is the primary visual
anchor: the object that turns "a hole in the ground" into "the secret cozy
home you can see glowing from outside."

### Allowed Charm Details

**Landmark Charm**
- MB-DOOR-01 — round blue entrance frame (#90CAF9) with warm interior light always visible spilling out (already a Locked Element, see World Identity Lock below)
- MB-FLOWERS-01 — the small round flowers growing on top of the entrance mound

**Surface Charm**
- MB-STONES-01 — the three round grey stepping stones leading to the entrance
- MB-GRASS-01 — the soft green grass above the burrow

**Small Prop Charm**
- MB-PILLOW-01 — the round blue plush pillow, the central interior element
- MB-CARROT-BOX-01 — the round orange carrot box with 2-3 carrots inside

**Motion Charm**
- the gentle sway of MB-FLOWERS-01 above the entrance
- Mimi's soft ear-flop and hop motion at the entrance threshold

### Forbidden Clutter

```
✗ dark or cold interior
✗ sharp edges anywhere
✗ large furniture or oversized objects
✗ frightening elements or creatures
✗ entrance that is not round or not blue
✗ metal or plastic materials
✗ loud sounds or sudden fast motion
✗ characters or logos in Hero View
```

### Thumbnail Hook

A round blue burrow entrance glowing with warm light beneath flower-topped
grass, with a stepping-stone path leading right up to the door.

### Child Recognition Test

**"Mimi'nin toprağın altındaki mavi kapılı gizli evi."** ("Mimi's secret blue-doored home under the ground.")

---

## Emotional Purpose (Özet)

Mimi'nin yuvası, çocukların "kendilerine ait bir yer" hissini yaşadığı sığınak. Yeraltında olmak korkutucu değil — gizli bir dünya, sadece onlara ait. Sıcak mavi ışık, yumuşak yastık ve havuç kutusu, "burası benim" hissini pekiştirir.

---

## Play Philosophy (Özet)

Mimi'nin yuvasında belirli bir aktivite yok — çocuklar kendi oyunlarını yaratır. Kitap okur, havuç yer, pencereden dışarı bakar, uyur. Kimse "şöyle yap" demez. Mimi'nin yuvası, çocukların kendi ritimlerinde dinlendiği, hayal kurduğu, sessiz kaldığı bir alan.

---

## Material Language

Yuvarlak giriş, yumuşak yastık, peluş dokular. Hiçbir köşe sivri, hiçbir yüzey sert değil. Mavi tonlar sıcak, beyazlar temiz, turuncu renkler canlı ama göz yormuyor. Her eşya küçük eller için tasarlanmış — dokunulası, kucaklanabilir, güvenli.

---

## Production Goal

This environment is designed as the canonical underground retreat for PomPom Hills. It will be reused across episodes for quiet moments, sleep scenes, friend visits, and cozy indoor activities.

---

## Hero View Technical Specification

| Parameter | Value |
|-----------|-------|
| Camera position | Dışarıdan girişe, hafif yukarıdan |
| Camera height | 0.80m |
| Lens | 28mm |
| Framing | Çimenlik %30, giriş %40, yol %30 |
| Horizon | Üstte çimenlik |
| Lighting direction | İçeriden sıcak ışık + dış mekan |
| Lighting softness | Çok yumuşak |
| Shadow density | %10-15 |
| Colour temperature | 5000K, sıcak |

---

## Camera Identity

| Parameter | Value |
|-----------|-------|
| Default lens | 28–35mm |
| Exterior height | 0.80m |
| Interior height | 0.60m |
| Movement philosophy | Yavaş, yumuşak, doğal |
| Allowed movements | Gentle pan, slow push-in |
| Forbidden movements | Fast zoom, shake, dutch angle |

---

## Lighting Identity

| Parameter | Value |
|-----------|-------|
| Primary direction | İçeriden sıcak ışık |
| Softness | Çok yumuşak |
| Ambient fill | Sıcak, yeraltı hissi |
| Shadow density | %10-15 |
| Exposure | Normal |
| Continuity | Sıcak ışık her zaman sabit |

---

## Colour Identity

| Parameter | Value |
|-----------|-------|
| White balance | 5000K, sıcak |
| Exposure | Normal |
| Contrast | Düşük, yumuşak |
| Saturation | Orta, pastel |
| Entrance colour | Mavi (#90CAF9) |
| Interior wall colour | Yumuşak mavi (#90CAF9) |
| Grass colour | Yeşil (#C8E6C9) |
| Carrot colour | Turuncu (#FF7043) |
| Stone colour | Gri (#E0E0E0) |
| Highlight behaviour | Yumuşak, sıcak |
| Shadow behaviour | Sıcak, yumuşak |
| Forbidden colour shifts | Soğuk mavi, koyu gri, siyah |

---

## Environmental Sound Identity

| Parameter | Value |
|-----------|-------|
| Wind | Hafif, dışarıdan |
| Birds | Uzaktan, sabah |
| Ambience level | Düşük, sessiz |
| Silence ratio | Yüksek — sessizlik baskın |
| Forbidden sounds | Gürültü, sert ses, modern teknoloji |

---

## Continuity Rules

| Element | Rule |
|---------|------|
| Giriş | Her zaman yuvarlak ve mavi |
| İç mekan | Her zaman sıcak ışık |
| Yastık | Her zaman yerinde |
| Havuç kutusu | Her zaman dolu |
| Pencere | Her zaman açık |
| Çiçekler | Her zaman üstte |
| Taş yol | Her zaman 3 taş |

---

## World Identity Lock

| Locked Element | Value | Acceptable Variations |
|----------------|-------|----------------------|
| Giriş şekli | Yuvarlak, mavi çerçeve | Hafif ton farkı |
| Giriş rengi | #90CAF9 | ±10% ton |
| İç mekan rengi | Yumuşak mavi | Mevsimsel变化 |
| Yastık rengi | Mavi | Uyumlu ton |

---

## Common Generation Failures

| Failure | Why It Matters | Fix |
|---------|---------------|-----|
| Karanlık iç mekan | Güven hissini bozar | Sıcak ışık ekle |
| Soğuk renk paleti | Kimliği bozar | Sıcak maviye ayarla |
| Keskin köşe | Preschool安全ına aykırı | Tüm kenarları yuvarla |
| Büyük eşyalar | Ölçek hatası | Küçük, çocuk boyutu |
| Korkutucu öğe | Çocukları korkutur | Reddet, yeniden üret |
| Giriş mavi değil | Kimliğin temeli kaybolur | Mavi çerçeveyi ekle |

---

## Video Generation Rules

| Element | Rule |
|---------|------|
| Kulak hareketi | Yumuşak, 10 frame döngü |
| Yastık hareketi | Çok yavaş, 20 frame |
| Kamera hızı | Maks %3 frame/sn pan veya tilt |
| Işık | Sabit, sıcak |

---

## Production Summary

Mimi's Burrow, PomPom Hills'in gizli sığınak mekanıdır. Yeraltında, sıcak ve güvenli bir dünya sunar. Mavi tonlar, peluş dokular ve küçük ölçekli eşyalarla tanınır. Sessiz anlar, uyku sahneleri ve arkadaş ziyaretleri için idealdir. Her üretimde sıcaklık ve güven hissi korunmalıdır.

---

## Production QA

```
✓ World Identity: Mimi's Burrow tanınabilir mi?
✓ Camera: Çocuk göz hizasında mı?
✓ Lighting: Sıcak ve yumuşak mı?
✓ Colour: Mavi + yeşil palet tutarlı mı?
✓ Atmosphere: Güvenli ve sıcak mı?
✓ World Recognition: Diğer mekanlardan ayrışıyor mu?
```

## Canonical Reusable Assets

| Asset ID | Description |
|----------|-------------|
| MB-DOOR-01 | Yuvarlak mavi giriş çerçevesi |
| MB-PILLOW-01 | Yuvarlak mavi peluş yastık |
| MB-WINDOW-01 | Küçük yuvarlak pencere |
| MB-SHELF-01 | Küçük kitaplık |
| MB-CARROT-BOX-01 | Yuvarlak turuncu havuç kutusu |
| MB-FLOWERS-01 | Üstteki çiçekler |
| MB-STONES-01 | Yuvarlak gri taşlar |
| MB-GRASS-01 | Yumuşak yeşil çimen |

## World Navigation

```
Dışarıdan giriş → Taş yolu → Mavi çerçeve → İç mekan → Yastık → Pencere → Kitaplık → Havuç kutusu
```

## View Transition Rules

### Default Sequence

```
1. Wide exterior (giriş)
2. Entrance close-up (mavi çerçeve)
3. Interior wide (iç mekan)
4. Close-up (yastık, havuç)
5. Window POV (dışarı bakış)
```

### When to Use

| Durum | Kullanım |
|-------|----------|
| Bölüm açılışı | Wide exterior |
| İçeri girme | Entrance close-up |
| Uyku sahnesi | Interior wide |
| Yemek sahnesi | Close-up |
| Kapanış | Window POV |

## Character Occupancy

| Shot Type | Max Characters |
|-----------|:--------------:|
| Wide exterior | 0 |
| Entrance | 1 |
| Interior wide | 2 |
| Close-up | 1-2 |

## Typical Episode Usage

### Story Types That Fit

- Uyku sahneleri
- Sessiz okuma
- Arkadaş ziyareti
- Gizli keşif

### Story Types That Don't Fit

- Hızlı hareket
- Gürültü
- Korku
- Rekabet

---

## Changelog

| Version | Changes | Breaking |
|---------|---------|:--------:|
| 1.0 | İlk oluşturma | — |
| 2.0 | Production pipeline standardına geçirme | — |
| 2.1 | Düzeltmeler | — |
| 3.0 | GLOBAL_ENVIRONMENT_STANDARD'a tam uyum — 35 bölüm, world-spec eklendi | ✅ Breaking |
| 3.1 | **Visual Richness Update.** Added `## Visual Richness & World Charm` section (Iconic Landmark = MB-DOOR-01, Allowed Charm Details by category, Forbidden Clutter, Thumbnail Hook, Child Recognition Test). Corrects over-simplification risk. No changes to World Identity Lock or pipeline. | — |

---

## Related Documents

| Document | Path |
|----------|------|
| World Spec | `POMPOM_HILLS_PRODUCTION/02_WORLDS/MIMIS_BURROW/02_WORLD_SPEC/05-mimis-burrow-world-spec.md` |
| Global Standard | `POMPOM_HILLS_PRODUCTION/00_GLOBAL_RULES/WORLD_STANDARDS/GLOBAL_ENVIRONMENT_STANDARD.md` |
| Image Prompts | `POMPOM_HILLS_PRODUCTION/00_GLOBAL_RULES/WORLD_STANDARDS/ENVIRONMENT_IMAGE_PROMPTS.md` |

---

*Bu belge Mimi's Burrow mekanı için creative referanstır.*
*Environment Bible v3.1*
*Son güncelleme: 4 Temmuz 2026*
