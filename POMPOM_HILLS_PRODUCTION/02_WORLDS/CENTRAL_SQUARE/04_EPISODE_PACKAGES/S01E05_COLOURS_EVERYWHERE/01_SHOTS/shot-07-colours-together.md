...
Kiko and Mimi are already present in the same Central Square set. They point back across
the same visible square, remembering the colours they found.
...
Start from the approved exported final continuity frame from Shot 06. Preserve the
...
Do not rearrange the square to show the colours. Use the existing set.

If not all colour objects are visible from the camera angle, characters may point
gently across the same square; do not reset the camera or move to a new location.
...
Kiko and Mimi stay in one clear open visible area. They may point toward remembered
colour objects across the square, but they must not walk from object to object, rearrange
...
Begin from the Shot 06 continuity frame with no camera reset. The first 1 second should visually hold close to @image1 before new action begins. After that, the remembering
gesture begins.

Use a calm child-eye-level medium-wide frame that keeps both characters visible and the
same Central Square set readable. Do not introduce new characters, new landmarks, new
paths, roads, shops, fountains, water features or a different Central Square area.
...
- Gentle pointing gesture rustle
- Soft Central Square ambience
- Light warm music bed beginning very quietly
