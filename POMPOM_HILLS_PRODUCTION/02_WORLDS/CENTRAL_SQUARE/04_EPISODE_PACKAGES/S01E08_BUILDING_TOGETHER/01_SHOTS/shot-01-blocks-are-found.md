...
- **Seeds:** all 6 building blocks (2 large flat + 4 medium round/flat), the build area on the ground near the Big Pompom Tree, Noah, Arda, warm morning daylight.
...
```text
Noah and Arda at Central Square in warm morning light, near the Big Pompom Tree. On the ground in front of them is a small set of exactly six colourful rounded building blocks — two large flat blocks and four medium round-and-flat blocks in soft pastel colours (coral, yellow, blue, green). Noah crouches and arranges the blocks, curious; Arda arrives from a rounded path at the edge of frame and comes to join him. Both are child-sized and soft, Arda a little smaller than Noah. Toy-set preschool look, rounded shapes, no other characters, {style} {camera} {lighting}
...
Begin with a static 35mm wide shot establishing Central Square and the six blocks on the ground. Arda enters from a rounded path and settles next to Noah. Camera gently settles toward the pair and the blocks while preserving the established camera distance — no pan, tracking, zoom or reset. Warm morning sunlight throughout. Avoid sudden zooms, shakes or whip pans.
...
| Time | Action | Camera |
|---|---|---|
| 0-1 sn | Continuity hold — Central Square, six blocks on the ground | Wide establishing |
...
- [ ] Object persistence — exactly 6 blocks visible, no more, no fewer
...
- [ ] **Gate A:** all 6 blocks + build area seeded and usable for later stacking; if any missing → NOT READY, fix this shot (not later prompts)
