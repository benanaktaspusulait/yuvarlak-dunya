# The Sleepy Leaf — Shot 14 — Book Closes Gently

---

## Scene Context

| Alan | Değer |
| --- | --- |
| Episode | The Sleepy Leaf (Opa's Storytime) |
| Shot | 14 / 20 |
| Duration | 15 seconds |
| Location | Central Square — Outdoor Story Blanket (sunset) |
| Characters | Opa, Kiko, Noah |

---

## Frame Lock

```text
Treat @image1 (Shot 13 final frame) as the complete visual master reference.
```

## Background Object Lock

Background locked from the first frame. Central Square elements remain identical. Only characters and the book may move.

## Visual Prompt

```text
Use Shot 13 video as Video Reference. Slow pull-back to a wide warm hold, Opa gently closes the picture book, Kiko and Noah smile, the blanket holds quietly, sunset light unchanged, {style} {camera} {lighting}

Match the lighting and colour grading exactly from the supplied continuity reference image.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No subtitles. No text.
```

---

## Camera Direction

Begin from the identical camera position of the Shot 13 reference. Slow pull-back to a wide hold. Lens 28mm, height 0.75m, slow pull-back (stays inside the existing scene).

---

## Dialogue

```
(Quiet warm hold — no dialogue)
```

---

## Shot Breakdown

| Time | Action | Camera |
|---|---|---|
| 0-1 sn | Continuity hold; Opa, Kiko, Noah in frame | Pull-back, 28mm |
| 1-3 sn | Opa gently closes the picture book | Pull-back, 28mm |
| 3-5 sn | Book settles on Opa's lap; soft sound | Pull-back, 28mm |
| 5-7 sn | Kiko and Noah smile softly at each other | Wide, 28mm, settle |
| 7-10 sn | Opa looks at both children with warmth; small blink | Wide, 28mm, settle |
| 10-12 sn | All three breathe gently; quiet group warmth | Wide, 28mm, static |
| 12-15 sn | Shared peaceful expression; continuity hold | Wide, 28mm, static |

---

## Natural Character Motion Rule

This shot should feel calm but alive.

Characters may use:
- blinking
- breathing
- small smiles
- gentle head turns
- small hand gestures
- one or two soft steps/hops
- looking at an object
- looking at another character
- pointing gently
- shared reaction
- leaning slightly
- whispered reaction
- shared glance

Do not freeze the characters.
Do not make the shot only a static talking pose.
Do not leave long empty pauses.
Do not make characters stare silently for several seconds.

---

## Sound

- Soft outdoor sunset ambience
- Gentle book-close sound
- Natural ambience only. No music. No background music.

---

## Lighting

Continue Shot 13 lighting exactly.

---

## Reference Usage

- Continuity Reference: Shot 13 video
- Voice: no dialogue in this shot

## Character Presence

Opa is already present at the beginning of the shot. Do not introduce any character after the shot has started.

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, readable text on book pages, photorealistic, horror, scary, dark lighting, violence, weapons, sharp objects, extra characters, redesigned environment, different tree, market stalls, modern street furniture, adult lecture feeling, different world, abrupt cut to black, static talking pose, characters frozen, frozen character, no movement, long empty pause, silent staring, awkward pause, static waiting, dead air, characters staring silently, frozen dialogue, inactive characters, random filler motion, meaningless movement, background music, music, melody, song, soundtrack, musical bed

## QA Checklist

- [ ] Frame Lock present
- [ ] Colour / lighting continuity verified
- [ ] Pull-back stays inside the scene
- [ ] Silent shot confirmed
- [ ] No on-screen text / subtitles
- [ ] Environment not reinterpreted
- [ ] No abrupt cut to black
- [ ] Shot feels calm but alive.
- [ ] Shot is not a static talking pose.
- [ ] No long empty pause occurs.
- [ ] No characters stare silently for several seconds.
- [ ] Every 2-3 seconds has a clear action, reaction, or dialogue beat.
- [ ] Dialogue is supported by physical action.
- [ ] Characters blink, breathe, gesture, turn, step, point, or react naturally.
- [ ] Existing continuity hold does not freeze the character unnaturally.
- [ ] Camera stability does not kill character motion.
- [ ] No random filler motion appears.

## Scale

Opa=120, Kiko=100, Noah=100 reference scale.
