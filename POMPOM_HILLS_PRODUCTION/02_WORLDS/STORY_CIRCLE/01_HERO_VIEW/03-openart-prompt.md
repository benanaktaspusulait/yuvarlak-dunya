# Story Circle — OpenArt World Builder Prompt
...
It is a 3D spatial extrapolation tool, not a prompt-following image generator. It reconstructs a navigable 3D space from images — it does not carefully honor a detailed spec the way our render pipeline does. A short text prompt cannot carry the precision of `02-world-spec.md` (exact hex colors, exact stone count, the full forbidden list). Some quality and identity loss versus the Bible/Spec standard should be expected.

**The correct input is not a fresh text prompt — it's the already-approved Hero View image.**

We already have a canon Hero View image that passed QA against the Bible. That image is the highest-fidelity representation of Story Circle we have. Feed *that* image into World Builder as the reference, and let the text prompt (below) serve only as a secondary hint — not as the thing trying to recreate the world from scratch.
...
If World Builder's 3D output still drifts from our identity (stones missing, light too cold, tree too small), that is an inherent limitation of the extrapolation — not something a better prompt can fully fix. Treat any World Builder output as a rough 3D blockout, and run it through the Production QA checklist before treating any capture from it as canon.
...
- does not understand internal asset IDs like `SC-TREE-01` or `SC-STONES-01`
...
```
A massive ancient storytelling tree with a wide spreading canopy of green leaves, casting warm amber dappled light onto the ground below. A ring of smooth round sitting stones is arranged in a circle beneath the tree, natural grey and brown, inviting and warm. A natural stone path leads toward the circle from the foreground. Soft green grass and scattered fallen leaves between the stones. Warm amber evening light filtering through the branches. Children's storybook style, soft matte handcrafted look, wise and safe atmosphere. No modern seating, no artificial lighting, no dark or ominous tone, no sharp corners, no noise or bustle.
```
...
- Image 1 (front): wide view of the great tree with the stone circle beneath
- Image 2 (back): view from inside the stone circle looking up at the canopy
- Image 3 (left): side view showing the path approaching the tree
- Image 4 (right, optional): close-up of the sitting stones with dappled light

All four images must share the same lighting direction, tree color, and stone colors so OpenArt reads them as one consistent space.
