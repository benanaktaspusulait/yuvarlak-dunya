# Render Prompts

Use the previous shot as Video Reference for all continuation shots.

# S01E02 — Render Prompts
## Global Standard

All render prompts follow `00-CORE/17_VIDEO_GENERATION_STANDARD.md`.


---

## Base Style

```
Pompom Hills v2.1, rounded preschool toy world, pastel colors, matte clay-plush surfaces, toddler-safe emotional clarity, Pixar-quality stylized 3D preschool illustration, soft global illumination, ambient occlusion, soft shadows
```

---

## Scene Prompts

### Scene 01 — Flower Hill — Home Cluster Zone
```text
Kiko opening the round door of her coral pink house on Flower Hill — Home Cluster Zone, rounded dome homes and garden path visible (use POMPOM_HILLS_PRODUCTION/02_WORLDS/FLOWER_HILL/01_HERO_VIEW/hero-view.png as visual quality anchor), morning warm soft daylight, light blue sky, round white clouds, warm sun, green grass, round flowers nearby, Kiko 100 units tall, brown pigtails, coral pink shirt, white shorts, happy smile, {style}, {camera}, {lighting}, 16:9
```

### Scene 02 — Flower Hill — Home Cluster Zone
```text
Kiko on Flower Hill — Home Cluster Zone looking at a round yellow ball on soft green grass near the rounded home cluster, curious expression, bending down, morning daylight, blue sky, round flowers nearby, {style}, {camera}, {lighting}, 16:9
```

### Scene 03 — Flower Hill — Home Cluster Zone
```text
Kiko holding a round yellow ball with both mitten hands, examining it curiously, then smiling, Flower Hill — Home Cluster Zone, rounded homes stable in background, green grass, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 04 — Flower Hill — Home Cluster Zone
```text
Kiko bouncing a round yellow ball on Flower Hill — Home Cluster Zone, ball rolling toward Mimi's blue burrow entrance, same rounded homes and garden path as previous shots, Kiko chasing happily, green grass, curved stepping stone path, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 05 — Flower Hill — Home Cluster Zone
```text
Mimi the soft blue rabbit appearing from his round blue burrow on Flower Hill — Home Cluster Zone, long floppy ears with pink inside, big green eyes, yellow t-shirt, looking at a round yellow ball at the entrance, Kiko standing nearby, coral pink shirt, morning daylight, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 06 — Flower Hill (transition from Home Cluster Zone) → Butterfly Meadow
```text
Kiko and Mimi running together from Flower Hill's Home Cluster Zone toward Butterfly Meadow, Kiko in coral pink shirt, Mimi hopping in soft blue, round yellow ball rolling ahead, green grass, curved path, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 07
```text
Kiko and Mimi standing in Butterfly Meadow surrounded by soft pastel butterflies, round colorful flowers, green grass, blue sky, one butterfly landing on Mimi's nose, wonder and delight expressions, {style}, {camera}, {lighting}, 16:9
```

### Scene 08
```text
Kiko and Mimi bouncing a round yellow ball together in Butterfly Meadow, both smiling and playing, pastel butterflies in background, green grass, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 09
```text
Kiko and Mimi watching a round yellow ball rolling away quickly across Butterfly Meadow, surprised expressions, green grass, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 10
```text
Kiko and Mimi chasing a round yellow ball toward Central Square, Big Pompom Tree visible in background, Kiko running, Mimi hopping, green grass, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 11
```text
Kiko and Mimi standing at Central Square looking at a round yellow ball that has stopped near the Big Pompom Tree, both smiling at each other, round colorful stones on ground, green grass, blue sky, {style}, {camera}, {lighting}, 16:9
```

### Scene 12
```text
Opa the wise owl sitting on a branch of his large pompom tree, looking down warmly at Kiko and Mimi below, both looking up and smiling, round yellow ball on the ground, Central Square, blue sky, {style}, {camera}, {lighting}, 16:9
```

---

## Negative Prompt

```
no sharp edges, no dark shadows, no realistic textures, no complex backgrounds, no scary elements, no harsh lighting, no neon colors, no busy compositions, no characters other than specified, no text, no watermarks
```
