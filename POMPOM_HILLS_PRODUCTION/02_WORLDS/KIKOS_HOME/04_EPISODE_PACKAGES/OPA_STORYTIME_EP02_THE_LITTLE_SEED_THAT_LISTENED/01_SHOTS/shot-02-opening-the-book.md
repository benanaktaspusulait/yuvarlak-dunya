# The Little Seed That Listened — Shot 02 — Opening the Book

---

## Scene Context

| Alan | Değer |
| --- | --- |
| Episode | The Little Seed That Listened (Opa's Storytime) |
| Shot | 02 / 20 |
| Duration | 15 seconds |
| Location | Kiko's Home — Reading Corner (indoor) |
| Characters | Opa, Kiko, Mimi |

---

## Frame Lock

```text
Treat @image1 (Shot 01 final frame) as the complete visual master reference.
Preserve framing, colour identity, lighting, exposure, white balance, environment identity, character proportions and performance.
The first visible frame must be visually indistinguishable from @image1. Treat @image1 as frame zero.
The animation begins only after the first frame has matched @image1 perfectly.
The viewer must not perceive a shot boundary.
```

## Background Object Lock

Background locked from the first frame. Round shelf with books, round rug, soft cushion, story chair, round window and coral curtain remain identical. Do not repaint or reinterpret. Only characters and the book may move.

## Visual Prompt

```text
Use Shot 01 video as Video Reference. Opa opens the rounded picture book, Kiko and Mimi lean in with curiosity, warm Kiko's Home light unchanged, medium framing, {style} {camera} {lighting}

Match the lighting and colour grading exactly from the supplied continuity reference image.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No subtitles. No text.
```

---

## Camera Direction

Begin from the identical camera position of the Shot 01 reference. Static medium. Lens 35mm, height 0.75m, static.

---

## Dialogue

```
(No dialogue — Opa opens the rounded book)
```

---

## Shot Breakdown

| Time | Action | Camera |
|---|---|---|
| 0-5 sn | Opa opens the rounded picture book. | Medium, 35mm, static |
| 5-10 sn | Kiko and Mimi lean forward with curiosity. | Medium, 35mm, static |
| 10-15 sn | Soft glow from the book page; children's eyes widen. | Medium, 35mm, static |

---

## Sound

- Soft indoor room tone
- Gentle page turn
- Warm ambience

---

## Lighting

Continue Shot 01 lighting exactly. Warm window light, soft ambient glow. Do not reinterpret — continue it.

---

## Reference Usage

- Continuity Reference: Shot 01 video (or exported final frame)
- Voice: same approved references for Opa, Kiko and Mimi

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, readable text on book pages, photorealistic, horror, scary, dark lighting, violence, weapons, sharp objects, extra characters, redesigned environment, modern furniture, harsh lighting, scary shadows, adult lecture feeling, different world

## QA Checklist

- [ ] Frame Lock present; first frame indistinguishable from @image1
- [ ] Colour / lighting continuity verified
- [ ] Characters already present (no re-entry)
- [ ] No readable text on book pages
- [ ] No on-screen text / subtitles
- [ ] Environment not reinterpreted
- [ ] No perceivable shot boundary

## Scale

Opa=120, Mimi=80, Kiko=100 reference scale (`00-CORE/VARIABLES.md`).
