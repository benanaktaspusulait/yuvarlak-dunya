# The Little Seed That Listened — Shot 08 — The Seed Waits

---

## Scene Context

| Alan | Değer |
| --- | --- |
| Episode | The Little Seed That Listened (Opa's Storytime) |
| Shot | 08 / 20 |
| Duration | 15 seconds |
| Location | Kiko's Home — Reading Corner (indoor) |
| Characters | Opa, Mimi |

---

## Frame Lock

```text
Treat @image1 (Shot 07 final frame) as the complete visual master reference.
```

## Background Object Lock

Background locked from the first frame. Reading Corner elements remain identical. Only characters and the book may move.

## Visual Prompt

```text
Use Shot 07 video as Video Reference. Medium shot, Opa continues the story, Mimi listens quietly, warm Kiko's Home light unchanged, {style} {camera} {lighting}

Match the lighting and colour grading exactly from the supplied continuity reference image.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No subtitles. No text.
```

---

## Camera Direction

Begin from the identical camera position of the Shot 07 reference. Static medium. Lens 35mm, height 0.75m, static.

---

## Dialogue

```
Opa: So the little seed waited, and kept listening.
Kiko: That's patient.
```

---

## Shot Breakdown

| Time | Action | Camera |
|---|---|---|
| 0-5 sn | Opa: "So the little seed waited, and kept listening." | Medium, 35mm, static |
| 5-10 sn | Kiko: "That's patient." | Medium, 35mm, static |
| 10-15 sn | Opa smiles. | Medium, 35mm, static |

---

## Sound

- Soft indoor room tone
- Gentle warm ambience

---

## Lighting

Continue Shot 07 lighting exactly.

---

## Reference Usage

- Continuity Reference: Shot 07 video
- Voice: same approved references for Opa and Kiko

## Character Presence

Opa is already present at the beginning of the shot. Do not introduce any character after the shot has started.

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, readable text on book pages, photorealistic, horror, scary, dark lighting, violence, weapons, sharp objects, extra characters, redesigned environment, modern furniture, harsh lighting, scary shadows, adult lecture feeling, different world

## QA Checklist

- [ ] Frame Lock present
- [ ] Voice continuity verified
- [ ] Colour / lighting continuity verified
- [ ] No readable text on book pages
- [ ] No on-screen text / subtitles
- [ ] Environment not reinterpreted
- [ ] No perceivable shot boundary

## Scale

Opa=120, Mimi=80 reference scale.
