# The Little Seed That Listened — Shot 01 — Welcome Home

---

## Scene Context

| Alan | Değer |
| --- | --- |
| Episode | The Little Seed That Listened (Opa's Storytime) |
| Shot | 01 / 08 |
| Duration | 15 seconds |
| Location | Kiko's Home — Reading Corner (indoor) |
| Characters | Kiko, Opa, Mimi |

Bu bölüm ortak açılış bumper'ından SONRA gelir — bkz. `POMPOM_HILLS_PRODUCTION/03_EPISODES/OPAS_STORYTIME_SERIES/OPENING_BUMPER/opening-bumper.md`.
Shot 01 aynı zamanda serinin kendi açılış beat'idir (`00-CORE/31_OPA_INTERACTIVE_STORYTELLING_FORMAT.md` §4).

---

## Background Object Lock

The background is locked from the first frame of this shot.

Maintain all visible background objects throughout the entire video. Do not remove, replace, repaint or transform any visible background object. The round shelf with books, round rug, soft cushion, story chair, round window and coral curtain remain identical in layout, colour and position. Do not introduce new objects. Do not reinterpret the Reading Corner during the shot.

Only the main characters and the picture book may move.

## Visual Prompt

```text
Use Kiko's Home Reading Corner (@image1) as LOCKED environment. Insert Opa, Kiko and Mimi (@image2/@image3/@image4) into the existing Reading Corner zone. Kiko welcomes Opa and Mimi, round shelf with books, soft cushion, warm window light, {style} {camera} {lighting}

Environment is LOCKED and identical to the world reference. Do not redesign the shelf, rug, window or curtain. Keep Kiko's Home Reading Corner identical in layout, colour and lighting.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No text.
```

---

## Camera Direction

Wide/medium establishing framing. Lens 28mm, height 0.75m, static.

Camera movement must remain slow, stable and natural. No fast zoom, no handheld shake, no whip pan.

---

## Dialogue

```
Kiko: Opa, come sit with us in my reading corner!
Opa: Come close, little friends.
Mimi: I love this corner.
```

---

## Shot Breakdown

| Time | Action | Camera |
|---|---|---|
| 0-5 sn | Kiko welcomes Opa and Mimi into her reading corner. | Wide, 28mm, static |
| 5-10 sn | Opa settles into the story chair, invites them close. | Wide, 28mm, static |
| 10-15 sn | Mimi settles on the cushion; Opa lifts the rounded picture book. | Wide, 28mm, static |

---

## Sound

- Soft indoor room tone (Reading Corner)
- Distant garden birdsong through the round window
- Very light page/cushion movement
- Gentle warm ambience (no music bed at shot level)

---

## Lighting

Warm morning light through the round window, cosy interior glow. Shadow side lifted, no hard shadows.

---

## Reference Usage

This is the FIRST story shot (after the shared opening bumper).

- World Reference: Kiko's Home — Reading Corner (`POMPOM_HILLS_PRODUCTION/02_WORLDS/KIKOS_HOME/`)
- Character References: Opa, Kiko, Mimi sheets

Loaded into OpenArt at production time; not hardcoded here.

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, readable text on book pages, photorealistic, horror, scary, dark lighting, violence, weapons, sharp objects, extra characters, redesigned environment, modern furniture, harsh lighting, scary shadows, adult lecture feeling, different world

## Opening Hook

The first 3–5 seconds must create curiosity: Kiko warmly waving Opa and Mimi into her cosy reading corner as Opa settles in and lifts a rounded book — "what story today?"

## QA Checklist

Reference: `16_VIDEO_QA_SPEC.md`, `00-CORE/SCENE_QA_CHECKLIST.md`

- [ ] Character integrity and consistency verified (Kiko, Opa, Mimi)
- [ ] Max 3 characters in frame
- [ ] Object persistence verified (shelf, rug, cushion, chair, window, curtain)
- [ ] Camera consistency verified (28mm, 0.75m, static)
- [ ] Lighting consistency verified (warm window light)
- [ ] No readable text on book pages
- [ ] No on-screen text / subtitles
- [ ] Environment not reinterpreted during shot
- [ ] Series opening beat present ("Come close, little friends.")
- [ ] Characters do not suddenly appear fully centered after the opening bumper.
- [ ] Characters are introduced by visible entrance, camera reveal or partial edge reveal (Character Introduction After Empty Opening Rule).
- [ ] No character pop-in occurs after the opening.

## Scale

Children small and childlike within the Reading Corner but readable for expression. Opa=120, Mimi=80, Kiko=100 reference scale (`00-CORE/VARIABLES.md`).
