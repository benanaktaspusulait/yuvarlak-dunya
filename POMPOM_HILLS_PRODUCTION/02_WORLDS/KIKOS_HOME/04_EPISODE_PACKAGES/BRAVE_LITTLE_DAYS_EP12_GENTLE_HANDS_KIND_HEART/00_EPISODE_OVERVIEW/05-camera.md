# EP12 — Camera Plan

## Episode: "Gentle Hands, Kind Heart"

---

## GLOBAL CAMERA RULES
- **Lens range:** 28–35mm equivalent
- **Height:** Child eye level (~80–100cm from ground)
- **Movement:** Slow, gentle — no fast pans or zooms
- **Style:** Observational, intimate, as if kneeling beside the characters
- **Focus:** Soft backgrounds, sharp subjects

---

## SHOT-BY-SHOT CAMERA

### Shot 01 — "A Little Butterfly"
| Property | Value |
|----------|-------|
| Lens | 35mm |
| Position | Wide establishing, child eye level |
| Movement | Slow push-in from wide to medium-wide |
| Subject | Kiko + butterfly on flower |
| Focus | Deep (both Kiko and flower visible) |
| Framing | Kiko screen-left, flower+butterfly screen-right |
| Duration | 15s |
| Notes | Establishes space. Warm garden light. Butterfly is small but noticeable. |

### Shot 02 — "I Want to Touch It"
| Property | Value |
|----------|-------|
| Lens | 30mm |
| Position | Medium shot, slightly low (child eye level) |
| Movement | Static; Kiko's lean remains inside the approved composition |
| Subject | Kiko — face and reaching hands |
| Focus | Medium (Kiko sharp, butterfly soft in BG) |
| Framing | Kiko center-left, reaching screen-right |
| Duration | 15s |
| Notes | Shows eagerness in body language. Hands reaching too fast. |

### Shot 03 — "Soft and Gentle"
| Property | Value |
|----------|-------|
| Lens | 32mm |
| Position | Medium two-shot, child eye level |
| Movement | Static |
| Subject | Kiko + Mimi together |
| Focus | Medium-deep (both characters sharp) |
| Framing | Kiko screen-left, Mimi screen-right, butterfly visible in BG |
| Duration | 15s |
| Notes | Coaching moment. Both faces readable. Warm, safe framing. |

### Shot 04 — "One Gentle Finger"
| Property | Value |
|----------|-------|
| Lens | 28mm (slightly wider for tension) |
| Position | Close-up, low angle looking up at finger + flower |
| Movement | Static; the finger movement creates the approach |
| Subject | Kiko's extended finger + butterfly on flower |
| Focus | Shallow (finger sharp, butterfly sharp, everything else soft) |
| Framing | Finger from screen-left, butterfly screen-right |
| Duration | 15s |
| Notes | Maximum suspense. The gap between finger and butterfly is the entire story. |

### Shot 05 — "It Lands on Me" ★ FRESH ANCHOR
| Property | Value |
|----------|-------|
| Lens | 30mm |
| Position | Locked close-up composed to include fingertip and Kiko's eyes |
| Movement | Static camera; rack focus only |
| Subject | Butterfly landing on finger → Kiko's amazed face |
| Focus | Rack focus from butterfly to Kiko's eyes |
| Framing | Finger and butterfly foreground, Kiko's amazed expression already inside frame |
| Duration | 15s |
| Notes | HIGHEST PRODUCTION VALUE. Magical moment. Time stops. Smooth, graceful camera. |

### Shot 06 — "So Beautiful"
| Property | Value |
|----------|-------|
| Lens | 32mm |
| Position | Medium shot, child eye level |
| Movement | Static |
| Subject | Kiko + Mimi + butterfly on finger |
| Focus | Medium (all three subjects readable) |
| Framing | Kiko center, Mimi screen-right, butterfly prominent |
| Duration | 15s |
| Notes | Warmth of connection. All three together. Joy without words. |

### Shot 07 — "Soft and Gentle" (whispered)
| Property | Value |
|----------|-------|
| Lens | 28mm (intimate) |
| Position | Close-up profile |
| Movement | Static |
| Subject | Kiko's face in profile, butterfly on finger foreground |
| Focus | Very shallow (Kiko's eye and butterfly sharp, rest soft bokeh) |
| Framing | Profile from screen-left, butterfly in soft foreground |
| Duration | 15s |
| Notes | Most intimate shot. Whispered phrase. Deep connection. |

### Shot 08 — "Gentle Hands, Kind Heart"
| Property | Value |
|----------|-------|
| Lens | 35mm |
| Position | Wide locked shot |
| Movement | Static; butterfly follows a graceful arc that remains inside frame |
| Subject | Butterfly departing → Kiko and Mimi watching |
| Focus | Deep (wide scene) |
| Framing | Kiko + Mimi bottom-center, butterfly flying up and away |
| Duration | 15s |
| Notes | Resolution. Butterfly free. Kiko peaceful. Eyes open final frame. |

---

## CAMERA MOVEMENT MAP
```
Shot 01: PUSH-IN (establishing → focus)
Shot 02: STATIC (eagerness stays inside frame)
Shot 03: STATIC (coaching stability)
Shot 04: STATIC (finger movement creates suspense)
Shot 05: STATIC + RACK FOCUS (magic reveal) ★
Shot 06: STATIC (warmth)
Shot 07: STATIC (intimacy)
Shot 08: STATIC WIDE (butterfly arc remains inside frame)
```

## LENS SEQUENCE
```
35mm → 30mm → 32mm → 28mm → 30mm → 32mm → 28mm → 35mm
Wide → Med → Med → Close → Close → Med → Close → Wide
```
Pattern: Opens wide, narrows to intimacy, returns to wide for resolution.
