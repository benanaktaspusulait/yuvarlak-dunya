# openart-shot-03-lets-make-bubbles.md

## Production Metadata

- Shot: 03/08
- Title: Let's Make Bubbles
- Duration: 15 seconds
- Continuity Mode: Block Continuation
- Block: A
- Lens: 32mm
- Camera Height: 0.75m
- Aspect Ratio: 16:9
- Target Resolution: 1920x1080
- Previous Video Reference: Shot 02
- Next Continuity Target: Shot 04

## Reference Upload Map

- @image1: Shot 02 approved final frame
- @image2: Canonical Kiko character reference
- @image3: Canonical Mimi character reference

---

## Hard Gate Before Generation

- [ ] @image1 is the approved Shot 02 final frame
- [ ] Character sides preserved
- [ ] Apple, table, window, rug stable
- [ ] Both characters visible with open eyes, mouths closed

---

## Video Prompt

MANDATORY SPOKEN DIALOGUE. Generate every line below clearly in English using the manually selected canonical character voices. Every quoted line is compulsory and must be spoken at the specified time. No omitted, shortened, combined, reordered or paraphrased lines. Never replace speech with silence, staring, ambience, sound effects or music.

Dialogue-priority generated audio. No music, birds, outdoor ambience, melody, song or soundtrack. Quiet motivated Foley only. Dialogue must remain clear and dominant.

Continue in the exact same cozy Kiko's Home living space, warm daytime. @image1 is the only visual authority. One continuous unbroken 15-second single take.

0.5–2.0 Mimi (MIMI voice) says clearly: "I have an idea!"
Mimi bounces excitedly, face lighting up with a playful idea. Kiko looks curious.

2.5–4.0 Kiko (KIKO voice) says clearly: "What kind of idea?"
Kiko's eyes widen slightly, a little less reluctant. She looks at Mimi with growing interest.

4.5–6.5 Mimi (MIMI voice) says clearly: "Let's make bubbles!"
Mimi does a small happy hop. Her hands mime making bubbles. Kiko's expression shifts to curiosity.

7.0–8.5 Kiko (KIKO voice) says clearly: "Bubbles? Really?"
Kiko leans forward slightly, eyes brightening. The reluctance is fading.

9.0–11.0 Mimi (MIMI voice) says clearly: "Yes! With soap and water!"
Mimi gently rubs her palms together, pretending to make bubbles. Do not create a real tap or soap bottle in the living space. Kiko smiles, genuinely interested now.

11.5–13.0 Kiko (KIKO voice) says clearly: "That sounds fun!"
Kiko nods, ready to try. Her body language shows willingness.

13.2–14.0 Mimi (MIMI voice) says clearly: "Come on, let's go!"
Mimi makes one small gesture toward the Washing Corner direction.

14.0–15.0 ACTION ONLY:
Kiko turns and completes one small controlled step in that direction, then settles into a stable standing pose. Do not make either character run. Do not show the Washing Corner. Do not transform the room. End on a stable frame with both characters in the Living Space. The final frame must not show Kiko mid-step, off balance or motion-blurred.

Camera: 32mm medium, slight ease-in on Mimi's excited gesture, then back to two-shot. Gentle moves only.

Do not display dialogue as on-screen text. No speech bubbles. No captions. No text.

---

## Compact OpenArt Video Prompt (paste ONLY this into OpenArt — ~826 chars)

826 characters

15s single take. @image1 only: same locked living space, Kiko screen-left, Mimi screen-right, intact apple centred on table. Do not create a tap, soap bottle or Washing Corner. Use the manually selected Kiko/Mimi voices. Speak every line exactly; omit or paraphrase none:
0.5 Mimi: "I have an idea!"
2.5 Kiko: "What kind of idea?"
4.5 Mimi: "Let's make bubbles!"
7.0 Kiko: "Bubbles? Really?"
9.0 Mimi: "Yes! With soap and water!"
11.5 Kiko: "That sounds fun!"
13.2 Mimi: "Come on, let's go!"
At 9.0 Mimi only rubs her palms, pretending to make bubbles. 14.0–15.0 action only: Kiko completes one small step toward the Washing Corner direction, then settles; no room change. End stable, eyes open, mouths closed. Dialogue dominant; no music, birds or ambience. Stable 32mm camera. No text, captions, subtitles or speech bubbles.

## Dialogue Timing Reference

| Time | Speaker | Line |
|------|---------|------|
| 0.5–2.0 | Mimi | "I have an idea!" |
| 2.5–4.0 | Kiko | "What kind of idea?" |
| 4.5–6.5 | Mimi | "Let's make bubbles!" |
| 7.0–8.5 | Kiko | "Bubbles? Really?" |
| 9.0–11.0 | Mimi | "Yes! With soap and water!" |
| 11.5–13.0 | Kiko | "That sounds fun!" |
| 13.2–14.0 | Mimi | "Come on, let's go!" |

Total spoken words: ~30

---

## Prop and Continuity Lock

- @image1: Shot 02 final frame
- Kiko: screen-left, curious expression replacing reluctance
- Mimi: screen-right, excited and playful
- Apple: intact, centre of table
- End with motivated movement toward Washing Corner (do not teleport)

---

## Audio and Voice Lock

- Kiko: Canonical saved Kiko voice — curious, warming up
- Mimi: Canonical saved Mimi voice — excited, playful
- Auto Voice: Off, Generated Dialogue: On, Generated Audio: On, Music: Off
- Dialogue-priority audio. No music, birds, outdoor ambience.
- Voices must be selected manually in the OpenArt interface.

---

## Negative Prompt

low quality, blurry, deformed, extra limbs, extra arms, extra hands, extra fingers, fused hands, floating hands, detached hands, character identity drift, character size drift, side swapping, prop duplication, duplicate apple, disappearing apple, new props, teleportation, location transformation, camera jump, sudden zoom, fast movement, frozen characters, static talking pose, silent staring, omitted dialogue, paraphrased dialogue, wrong speaker, voice swapping, simultaneous lip movement, music, song, melody, soundtrack, birds, outdoor ambience, captions, subtitles, speech bubbles, on-screen text, watermark, HDR, glossy plastic, excessive contrast, excessive saturation, excessive sharpness, harsh shadows, orange shift, green tint, blue tint, photorealism, horror, crying, tantrum, force, pressure

---

## Final QA Checklist

- [ ] @image1 is Shot 02 final frame
- [ ] Mimi's excitement reads as playful (not manic)
- [ ] Kiko's shift from reluctant to curious is gradual
- [ ] Apple untouched on table
- [ ] End with motivated movement toward Washing Corner
- [ ] Do not end on a blurred or mid-step frame
- [ ] No teleportation, no Washing Corner visible
- [ ] All 7 dialogue lines spoken
- [ ] No on-screen text, no music
- [ ] Stable camera
- [ ] Eyes open first and final frames
