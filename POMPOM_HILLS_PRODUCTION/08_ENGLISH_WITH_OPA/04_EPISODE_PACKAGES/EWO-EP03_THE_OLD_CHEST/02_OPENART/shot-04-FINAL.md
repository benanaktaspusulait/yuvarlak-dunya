# Shot 04 — What Colour Is It?

## Production Mode
LINKED CONTINUITY SHOT — from Shot 03.
`@image1` = mandatory approved final frame of Shot 03 (tiny bird in Arda's palm, all three looking at it).
Duration: exactly 15 seconds. Aspect ratio: 9:16 vertical. Frame-to-Video.
Auto Voice OFF. Canonical voices selected manually. Prompt Enhancer OFF. Cinematic/Auto Camera OFF. No OpenArt music.

## Story Context
The fourth object is a round blue glass marble — smooth, beautiful, catches the light. Rolling it gently between them creates natural colour and shape vocabulary. The marble's light effects are visually magical.

## Canonical Character Locks
**Opa:** Large round light-green owl. No human hands — rounded wings only. Golden glasses, brown scarf.
**Arda:** Sky-blue shirt, light-blue trousers, tan shoes.
**Luca:** Cream shirt with green sleeves, khaki shorts, orange trainers.

## Location Lock
Same platform. Key, stone and tiny bird now placed beside the open chest.

## Prop and Object Continuity Lock
- Key, stone, tiny bird: all visible beside chest.
- Round blue glass marble: fourth object. Clear blue, catches light beautifully.

## Spoken Language and Voice Lock
English only. One speaker at a time. No overlap. First line by 1.5s.

## Exact Dialogue
1. Luca: "What is this one?"
2. Arda: "A ball? Is it a ball?"
3. Opa: "It's a marble. What colour is it?"
4. Luca: "It's blue!"
5. Arda: "Round and blue!"
6. Luca: "It catches the light!"
7. Opa: "Yes. Round, blue and smooth."
8. Arda: "I love it."

**Word count:** 29 words. **Turn count:** 8 turns.

## OpenArt Video Prompt
AUDIO PRIORITY: English only. Canonical voices only.
Continue from `@image1`. Same platform. Preserve characters and matte palette 1:1. No HDR.

**0.0–2.0s** Opa reaches into the chest with a rounded wing and carefully lifts out a round blue glass marble. It catches the light immediately. Luca — **"What is this one?"**
**2.0–4.0s** Arda leans in — **"A ball? Is it a ball?"**
**4.0–7.0s** Opa holds the marble on a flat wing — **"It's a marble. What colour is it?"**
**7.0–8.5s** Luca points — **"It's blue!"**
**8.5–10.0s** Arda gestures its round shape — **"Round and blue!"**
**10.0–12.0s** Luca watches as the marble catches a beam of light — **"It catches the light!"**
**12.0–13.5s** Opa nods softly — **"Yes. Round, blue and smooth."**
**13.5–15.0s** Arda gazes at it — **"I love it."** Final frame: marble resting between all three, light glinting on its surface, wonder on everyone's faces.

## Exact 0.0–15.0 Second Timeline
| Time | Action | Speaker | Line |
|------|--------|---------|------|
| 0.0–2.0s | Marble revealed from chest | Luca | "What is this one?" |
| 2.0–4.0s | Arda guesses | Arda | "A ball? Is it a ball?" |
| 4.0–7.0s | Opa holds marble on flat wing | Opa | "It's a marble. What colour is it?" |
| 7.0–8.5s | Luca identifies colour | Luca | "It's blue!" |
| 8.5–10.0s | Arda describes shape | Arda | "Round and blue!" |
| 10.0–12.0s | Light catches marble | Luca | "It catches the light!" |
| 12.0–13.5s | Opa describes further | Opa | "Yes. Round, blue and smooth." |
| 13.5–15.0s | Arda's reaction | Arda | "I love it." |

## Camera and Framing Lock
Child-height camera. Marble visible with light effects. Not too close — keep all three characters readable. Final frame stable for Shot 05 link.

## Sound / SFX Lock
No OpenArt music. Ambient: birds, breeze, gentle marble surface sounds. Never overpower dialogue.

## Visual Continuity / Style Lock
Soft matte 3D. Marble light effects gentle, not overwhelming. Medium-low contrast. No HDR.

## Post-Production Learning Metadata — DO NOT GENERATE IN OPENART
- Target phrase: "What colour is it?" / "It's round and blue."
- Vocabulary: colour, blue, round, smooth, marble

## Negative / Hard Locks
No human hands on Opa. No missing glasses or scarf. No exaggerated light effects. No HDR. No generated text. No non-English speech.

## Approval Gate
- Opens from Shot 03 final frame.
- Blue marble clearly visible with gentle light catch.
- 29 words, 8 turns. No human anatomy on Opa.
