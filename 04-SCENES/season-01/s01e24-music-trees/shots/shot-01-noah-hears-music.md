# Music Trees — Shot 01 — Noah Hears Music

---

## Scene Context

| Alan | Değer |
| --- | --- |
| Episode | Music Trees |
| Shot | 01 / 04 |
| Duration | 15 seconds |
| Location | [Mekan] |
| Characters | [Karakterler] |

---

## Visual Prompt

```text
Noah hears music from the trees. in Pompom Hills, warm pastel colors, round soft shapes, night sky with stars, Medium, 50mm, static, no other characters, {style} {camera} {lighting}

Do not display dialogue as on-screen text. No speech bubbles. No captions. No subtitles. No text.
```

---

## Camera Direction

The shot must begin from the identical camera position shown in @image1.

No camera movement is allowed until the first frame matches @image1 exactly.

The first visible frame should appear visually identical to @image1.

Only then may the planned camera movement begin.

Medium, 50mm, static.

Camera movement must remain slow, stable and natural.

---

## Dialogue

```
Noah: Listen! The trees are singing!
```

---

## Shot Breakdown

| Time | Action | Camera |
|---|---|---|
| 0-5 sn | Noah hears music from the trees. | Medium, 50mm, static |
| 5-10 sn | Character interaction | Medium, 50mm, static |
| 10-15 sn | Key moment | Medium, 50mm, static |

---

## Sound

- Crickets chirping (night)
- Soft night wind
- Gentle ambient sounds

---

## Lighting

Warm moonlight or soft daylight.

Soft, gentle glow.

---

## Negative Prompt

low quality, blurry, deformed, extra limbs, text, watermark, photorealistic, horror, scary, dark lighting, violence, weapons, sharp objects, extra characters, redesigned environment
